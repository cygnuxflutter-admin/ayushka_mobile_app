...
class ChildrenHierarchy extends GetView<CowsDetailScreenController> {
  const ChildrenHierarchy({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        ...
        title: "Children Hierarchy",
        styleType: Style.bgFillBluegray900,
      ),
      body: SafeArea(
        child: Obx(
          () {
            switch (controller.childrenHierarchyStatus.value) {
                          ...
                          Expanded(
                            child: Center(
                              child: InteractiveViewer(
                                ...
                                minScale: 0.01,
                                maxScale: 2.0,
                                child: controller.childrenEdgesList.isNotEmpty
                                    ? GraphView(
                                        animated: true,
                                        graph: controller.childrenGraph.value,
                                        algorithm: BuchheimWalkerAlgorithm(
                                          controller.childrenBuilder,
                                          TreeEdgeRenderer(controller.childrenBuilder),
                                        ),
                                        paint: Paint()
                                          ..color = Colors.green
                                          ..strokeWidth = 1
                                        ...
                                        builder: (Node node) {
                                          ...
                                          var nodes = controller.childrenNodes;
                                          if (nodes.isEmpty) {
                                    ...
                                    : controller.foundCow != null ? rectangleWidget(
                                        id: controller.foundCow!.tagId,
                                        breed: controller.foundCow!.breed,
                                        type: controller.foundCow!.type,
                                        label: controller.foundCow!.calfName,
                                        gender: controller.foundCow!.isFemale == true ? "Female" : "Male",
                                      ) : const SizedBox(),
                              ),
                            ),
                          ),
